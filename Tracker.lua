...
local function makePanel(name,parent,isMap)
  ...
  panel.previous=CreateFrame("Button",nil,panel);panel.previous:SetWidth(28);panel.previous:SetHeight(22);panel.previous:SetPoint("BOTTOMLEFT",panel,"BOTTOMLEFT",9,3)
  ...
  panel.next=CreateFrame("Button",nil,panel);panel.next:SetWidth(28);panel.next:SetHeight(22);panel.next:SetPoint("BOTTOMRIGHT",panel,"BOTTOMRIGHT",-9,3)
...
function Q:RenderTracker(panel,entries,zone,zoneName)
  ...
  if panel.isMap then
    ...
    if panel.heading:GetStringWidth()>138 then panel.heading:SetText(prefix.."Zone") end
  ...
  if closed then panel:SetHeight(33);return end
  -- Five entries per page keeps the classic tracker compact; all objectives wrap.
  panel.pages=math.max(1,math.ceil(getn(entries)/5));panel.page=math.min(panel.pages,panel.page)
  local top=37
  for index=(panel.page-1)*5+1,math.min(panel.page*5,getn(entries)) do
    local slot=index-(panel.page-1)*5
  ...
  panel:SetHeight(top+(panel.pages>1 and 28 or 9))
