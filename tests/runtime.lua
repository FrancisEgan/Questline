...
local function trackerModeTests(Q)
  ...
  expect(QuestlineSettings.trackerMode=="world" and Q.tracker.heading:GetText()=="Quests - World","existing settings default to World")
  ...
  Q:Select(entries[12].key);expect(Q.tracker.page==1 and Q.tracker.rows[1].entry==entries[12],"World selection promotes the highlighted quest to page one")
  ...
  expect(#Q:GetTrackerEntries()==6 and Q.tracker.rows[1].entry.number==12,"Zone preserves shared quest numbers and excludes completed quests with off-zone turn-ins")
  expect(QuestlineSettings.selected==entries[12].key,"mode switch preserves highlighted quest")
  Q:Select(entries[12].key)
  expect(Q.tracker.page==1 and Q.tracker.rows[1].entry==entries[12],"selection leads the filtered entries on page one")
  ...
  expect(Q.mapTracker.rows[1].badge.text:GetText()=="?","selected completed quest leads its turn-in zone")
  ...
  expect(Q.mapTracker.page==1 and Q.mapTracker.rows[1].entry==entries[14],"HUD mode toggle preserves map filtering and page")
...
local function trackerClickTests(Q)
  ...
  click(Q.mapTracker.rows[1],"RightButton")
  ...
  click(Q.mapTracker.rows[2].badge)
...
local function questLevelAndRateTests(Q)
  ...
  expect(Q:Normalize(Q.tracker.rows[2].title:GetText())=="[8] the mills overrun","normal tracker displays quest level before its title after the highlighted quest")
  SetMapZoom(1,1);WorldMapFrame:Show();Q:RefreshMap()
  expect(Q.mapTracker.rows[1].entry.id==845 and Q.mapTracker.rows[2].entry.id==844,"zone-filtered map tracker prioritizes highlights before other quests")
  local mapped
  ...
  expect(Q.tracker.rows[2].title:GetText():find("|cff7f7f7f8|r",1,true),"level-up events refresh difficulty colors in visible trackers")
...
local function multiSelectionTests(Q)
  ...
  expect(selectedCount()==1 and Q:IsSelected("multi:3") and Q.tracker.rows[1].entry.key=="multi:3" and Q.mapTracker.rows[1].entry.key=="multi:3","normal map click replaces the group with one highlighted quest at the front")
...
local function partySyncTests(Q)
  ...
  expect(findLine("    Bob: On this quest; progress unavailable"),"non-addon member gets shared-quest status without invented counters")
  ...
  expect(Q:PartyObjectiveLines(entry,1,"Leg Meat - 1/2",false,"")[3].text=="  Alice: Progress unavailable","different required counts are not presented as matching objectives")
  deliver(40535,body,7);now=now+91
  expect(Q:PartyObjectiveLines(entry,1,"Leg Meat - 1/2",false,"")[3].text=="  Alice: Progress outdated","old counters are explicitly marked outdated rather than displayed as live")
  ...
  expect(Q:PartyObjectiveLines(entry,1,"Leg Meat - 1/2",false,"")[3].text=="  Alice: Offline","disconnected members show Offline")
  connected.Alice=true;Q:RefreshPartyRoster()
  expect(Q:PartyObjectiveLines(entry,1,"Leg Meat - 1/2",false,"")[3].text=="  Alice: On this quest; progress unavailable","reconnecting does not revive counters from before disconnect")
  deliver(0,"40535,426",10);deliver(40535,body,11)
  deliver(0,"426",12)
  local members=Q:GetPartyQuestMembers(entry)
  expect(#members==1 and members[1].name=="Bob","remote removal clears a quest even while native membership still reports it shared")
